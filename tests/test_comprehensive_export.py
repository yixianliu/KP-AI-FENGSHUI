...
class FakeAgnes:
    ...
    def __init__(self, content=VALID_COMPREHENSIVE_JSON, disclaimer_as_list=False):
        ...
        Args:
            content: 直接作为模型返回的 JSON 字符串（默认用合法综合建议 JSON）。
            disclaimer_as_list: 为 True 时把 disclaimer 伪装成列表，用于验证
                流水线对返回类型的兜转修复。
        """
    ...
    def chat_completion(self, messages, temperature=0.4, max_tokens=4096):
        """模拟模型对话接口，返回预定 content 与 token 用量。

        不发起任何网络请求；disclaimer_as_list 为 True 时把 disclaimer 临时
        改为列表以触发类型兜转分支。

        Args:
            messages: 提示词消息列表（本桩忽略内容）。
            temperature: 采样温度（桩忽略）。
            max_tokens: 最大生成长度（桩忽略）。

        Returns:
            dict: 形如 {'content': ..., 'usage': {...}} 的响应。
        """
...
class TestComprehensivePipeline(unittest.TestCase):
    ...
    def setUp(self):
        """清空缓存防互相污染，并注入 FakeAgnes 作为流水线客户端。"""
